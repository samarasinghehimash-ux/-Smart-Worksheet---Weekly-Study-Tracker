...
def display_full_analytics(df, start_date):
    ...
    if not week_df.empty:
        ...
        st.pyplot(fig)
    else: st.warning("මෙම කාල සීමාව සඳහා දත්ත නැත.")
...
if st.session_state.logged_in:
    ...
    if st.session_state.username == "admin":
        ...
        with tab1:
            ...
            for i in range(3):
                ...
                # පැය සහ මිනිත්තු වෙනම Input ලෙස
                col_h, col_m = st.sidebar.columns(2)
                h = col_h.number_input("පැය (Hours)", 0, 20, key=f"h{i}")
                m = col_m.number_input("මිනිත්තු (Mins)", 0, 59, key=f"m{i}")
                
                names.append(n)
                # මිනිත්තු පැය බවට පරිවර්තනය කර එකතු කිරීම
                hours_final.append(h + (m/60))

            if st.sidebar.button("දත්ත සුරකින්න (SAVE DATA)"):
                conn_db.execute('''INSERT INTO study_logs VALUES(?,?,?,?,?,?,?,?,?) 
                                   ON CONFLICT(username, date) DO UPDATE SET 
                                   sub1_h=excluded.sub1_h, sub2_h=excluded.sub2_h, sub3_h=excluded.sub3_h''', 
                                (st.session_state.username, str(entry_date), stream, names[0], hours_final[0], names[1], hours_final[1], names[2], hours_final[2]))
                conn_db.commit()
                st.rerun()
            
            st.sidebar.divider()
            # අද දත්ත මැකීමේ බොත්තම
            if st.sidebar.button("🗑️ අද දත්ත මකන්න (Delete Today)"):
                conn_db.execute(f"DELETE FROM study_logs WHERE username='{st.session_state.username}' AND date='{entry_date}'")
                conn_db.commit()
                st.rerun()
            
            # නව බොත්තම: මුළු සතියේම දත්ත මැකීමට
            st.sidebar.subheader("🚨 දත්ත කළමනාකරණය")
            if st.sidebar.button("🚨 සතියේ සියලු දත්ත මකන්න"):
                # තෝරාගත් දින සිට සතියේ අවසානය දක්වා දත්ත මකයි
                end_range = entry_date + timedelta(days=6)
                conn_db.execute(f"DELETE FROM study_logs WHERE username='{st.session_state.username}' AND date BETWEEN '{entry_date}' AND '{end_range}'")
                conn_db.commit()
                st.success("සතියේ දත්ත මකා දැමීම සාර්ථකයි!")
                st.rerun()
    ...
    st.info("පද්ධතිය භාවිතා කිරීමට Login වන්න.")import streamlit as st
import sqlite3
import pandas as pd
import matplotlib.pyplot as plt
import numpy as np
