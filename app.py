...
def display_full_analytics(df, start_date):
    ...
    if not week_df.empty:
        ...
        # Feedback
        if total_h < 40:
            st.markdown(f'<div class="feedback-box" style="background-color: #ffebee; color: #c62828; border-color: #ef9a9a;">😟 ඔබ තවමත් දුර්වල මට්ටමක සිටින්නේ. තව ගොඩක් මහන්සි වෙන්න!</div>', unsafe_allow_html=True)
        elif total_h < 60:
            st.markdown(f'<div class="feedback-box" style="background-color: #e8f5e9; color: #2e7d32; border-color: #a5d6a7;">🙂 ඉතා හොඳයි! තව උත්සාහ කරන්න, ඔබට පුළුවන්!</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="feedback-box" style="background-color: #fff8e1; color: #ff8f00; border-color: #ffe082;">🔥 සුපිරි! ඔබ ඉතා දක්ෂ ලෙස වැඩ කර තිබෙනවා!</div>', unsafe_allow_html=True)
...
if st.session_state.logged_in:
    ...
    st.markdown("<div style='text-align: center;'>Concept by: <b>Charaka Dhananjaya</b> | Developed by: <b>Hiratrix IT Solutions</b></div>", unsafe_allow_html=True)
    st.divider()
    ...
    if st.session_state.username == "admin":
        ...
        if not all_logs.empty:
            ...
            sel_u = st.selectbox("ශිෂ්‍යයා", all_logs['username'].unique())
            adm_d = st.date_input("ආරම්භක දිනය", datetime.now() - timedelta(days=6))
